...
namespace MiBud.Droid.Dependencies
{
    public class Bluetooth_Android : IBlueToothDevices
    {
        ...
        public async Task<string> GetFirmwareVersion(string dongle_type, string txHeader, string rxHeader, string protocol)
        {
            ...
            var firmware_version = await DongleCommunication("500C47568AFE56214E238000FFC3", dongle_type, txHeader, rxHeader, protocol);
            return firmware_version;
        }

        private async Task<string> DongleCommunication(string sec_cmd, string dongle_type, string txHeader, string rxHeader, string protocol)
        {
            try
            {
        ...
        public async Task<ObservableCollection<Models.ReadPidPresponseModel>> ReadPid(ObservableCollection<Models.ReadParameterPID> pidList)
        {
            try
            {
                ...
                var respo = await Task.Run(async () =>
                {
                    if (App.dongle == "24v")
                    {
                        result = await dSDiagnostic24.ReadParameters(pidList.Count, list);
                    }
                    else
                    {
                        result = await dSDiagnosticELM.ReadParameters(pidList.Count, list);
                    }
